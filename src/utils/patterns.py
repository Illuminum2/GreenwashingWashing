...
class Patterns:
    @staticmethod
    def compile(pattern: str | None, flags: re._FlagsType = 0) -> re.Pattern:
        if pattern:
            return re.compile(pattern, flags)
        return re.compile(r"(?!)") # Pattern matches nothing, empty string matches everything
    ...
    @staticmethod
    def compile_patterns_iu(patterns: list[str] | None) -> re.Pattern:
